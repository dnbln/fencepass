...
struct CFPath {
  // blocks visited, except for the block containing the instruction we started
  // from
...
void floodPaths(AAResults &alias_analysis_results, Value *initial_value,
                Map &map, CFPath &currentPath, BasicBlock *bb,
                BasicBlock::iterator bbIter, const bool fenceWithStore,
                const bool fenceWithLoad) {
  if (bbIter == bb->end()) {
    for (const auto succ : successors(bb)) {
      if (currentPath.visited_basic_blocks.find(succ) !=
          currentPath.visited_basic_blocks.end()) {
        // if we didnt find any conflicts the first time going through this
        // block, we sure won't find any conflicts going through it a second
        // time.
        continue;
      }
      currentPath.visited_basic_blocks.insert(succ);
      CFPath cpClone = currentPath;
      floodPaths(alias_analysis_results, initial_value, map, cpClone, succ,
                 succ->begin(), fenceWithStore, fenceWithLoad);
  ...
  if (isMemoryAccess(I)) {
    ...
    case Load:
    case AtomicRMW:
      if (fenceWithLoad && doCommitByAAResult(alias_analysis_results.alias(
                               initial_value, newValue))) {
        map.commitPath(currentPath);
      }
      break;
    case Store:
      if (fenceWithStore && doCommitByAAResult(alias_analysis_results.alias(
                                initial_value, newValue))) {
        map.commitPath(currentPath);
      }
      break;
  ...
  ++bbIter;
  floodPaths(alias_analysis_results, initial_value, map, currentPath, bb,
             bbIter, fenceWithStore, fenceWithLoad);
}

Map runCFAnalysis(Function &F, const bool allowStoreStoreReordering,
  ...
  for (auto &BB : F) {
    for (auto it = BB.begin(); it != BB.end(); ++it) {
      ...
      floodPaths(alias_analysis_results, ptr, map, current, &BB, passedIt,
...
struct FencePass : PassInfoMixin<FencePass> {
  ...
  FencePass(bool allowStoreStoreReordering)
      : allowStoreStoreReordering(allowStoreStoreReordering) {}
